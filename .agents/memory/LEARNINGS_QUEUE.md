...
| `LRN-015` | 2026-09-16 | Codex/Astra Review Discrepancy & Invariant Recovery | Intentional, test-enforced protocol invariants (such as `_startRound` solvency debt queuing, `registerVoterWithSignature` relayer exclusion, and `updateCreditLimit` capacity reservation bounds) take precedence over reviewer model priors. Never patch code to satisfy external model flags when existing tests explicitly assert that behavior as intended protocol design. | [.agents/AGENTS.md](file:///c:/Users/Josh/Desktop/PBMRebateTreasuryFinal/.agents/AGENTS.md) | `PENDING_REVIEW` |

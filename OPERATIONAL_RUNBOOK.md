...
- **Emergency Revocation:** If a relayer key is suspected of being compromised:
  1. The Council must immediately invoke `updateRelayerStatus` on the `PatientFundParticipatoryBudgeting` contract to revoke the compromised relayer address:
     ```solidity
     // Call from Council multisig
     PatientFundParticipatoryBudgeting.updateRelayerStatus(compromisedRelayerAddress, false);
     ```
  2. Register a new secure relayer key using:
     ```solidity
     PatientFundParticipatoryBudgeting.updateRelayerStatus(newRelayerAddress, true);
     ```
  3. All outstanding signatures generated by the compromised key are invalidated upon revoking the relayer status.

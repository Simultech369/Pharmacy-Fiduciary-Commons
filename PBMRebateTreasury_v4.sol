    ...
    Pausable
{
     ...
     * @param _initialDailyCap   Starting daily volume cap (hard cap set to 10×).
    ...
    /**
     * @notice Claims allocated tokens for the current epoch for multiple pharmacies in
     *         one transaction. Intended for a delegated claims agent doing batch enrollment.
     * @dev    All arrays must be equal length. Each entry claims for msg.sender against
     *         the current epoch. This function processes multiple distinct proofs for the
     *         same epoch (e.g., split-cap scenarios) rather than across multiple epochs.
     *         All arrays must be equal length.
     *
     * @param amounts      Gross allocated amounts per claim entry.
     * @param eligibleCaps Per-pharmacy caps per claim entry.
     * @param proofs       Merkle proofs per claim entry.
     */
    function claimBatch(
    ...
    function _processClaim(
        ...
        internal
    {
        uint256 epoch = currentEpoch; // cache SLOAD — used 6× below
        ...
        whenNotPaused
    {
        ...
        if (block.timestamp < epochStartTimestamp + MIN_EPOCH_DURATION) revert("Epoch too short");
        if (epochVolume < MIN_EPOCH_VOLUME) revert("Volume too low");

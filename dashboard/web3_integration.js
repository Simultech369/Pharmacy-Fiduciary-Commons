...
function updateProxyStatusBadge(text, className = "badge-unverified-slate") {
...
function setDashboardLocked(locked, warningMessage) {
  ...
  if (warningBanner) {
    if (locked) {
      ...
      if (warningMessage) {
        warningBanner.querySelector("span").innerText = `Network warning: ${warningMessage}`;
      }
    } else {
    ...
    try {
      ...
      if (warningBanner) {
        if (actualBalance < requiredBalance) {
          ...
          let msg = `Liquidity deficit: the actual contract balance (${actualBalance.toFixed(2)}) is lower than the required accounting balance (${requiredBalance.toFixed(2)}).`;
          if (roundState === 1) {
          ...
          warningBanner.innerText = "Warning: active round has zero liquidity in matching pool.";
        } else if (recycledPool === 0 && roundState === 0) {
          warningBanner.style.display = "block";
          warningBanner.style.background = "rgba(245, 158, 11, 0.1)";
          warningBanner.style.borderColor = "var(--accent-orange)";
          warningBanner.style.color = "var(--accent-orange)";
          warningBanner.innerText = "Warning: zero liquidity in recycled pool.";
        } else {
          warningBanner.style.display = "none";
        }
      }
...
window.syncProxyProfile = async function() {
  ...
  statusEl.textContent = "Syncing voter profile to proxy database...";
  updateProxyStatusBadge("PROXY SYNC IN PROGRESS", "badge-unverified-slate");
  ...
  try {
    ...
    try {
      ...
      if (healthResponse.ok) {
        ...
        if (healthData.proxyAddress) {
          ...
          updateProxyStatusBadge("LOCAL PROXY REACHABLE", "badge-status-emerald");
        }
      }
    } catch (err) {
      console.warn("Could not query proxyAddress from health check, falling back:", err.message);
      updateProxyStatusBadge("PROXY HEALTH UNCHECKED", "badge-unverified-slate");
    }
    ...
    statusEl.textContent = "Database profile synced.";
    updateProxyStatusBadge("PROXY SYNC RECORDED", "badge-status-emerald");
    setTimeout(() => {
    ...
    statusEl.textContent = `Profile sync failed: ${dbErr.message}. `;
    updateProxyStatusBadge("PROXY SYNC FAILED", "badge-unverified");
    const retryBtn = document.createElement("button");
    retryBtn.className = "btn-vote retry-sync-button";
  ...
  try {
    ...
    if (displayEvents.length === 0 && eventListEl.children.length === 0) {
      ...
      for (const ev of displayEvents) {
        ...
        nameSpan.classList.add("receipt-name-onchain");
...
window.generateVisualReceiptSample = function() {
  ...
  window.sampleReceiptData = {
    ...
    privacyBlinding: {
      credentialPepper: "server-side-pepper-placeholder",
    ...
    databaseLedgerState: {
      ...
      status: "completed",
      lockedLease: "15s row-lock status sample",
  ...
  document.getElementById("step-1-badge").textContent = "Sample: EIP-191";
  document.getElementById("step-1-badge").style.color = "#4ade80";

  document.getElementById("step-2-badge").textContent = "Sample: EIP-712";
  document.getElementById("step-2-badge").style.color = "#4ade80";

  document.getElementById("step-3-badge").textContent = "Sample: HMAC-SHA256";
  document.getElementById("step-3-badge").style.color = "#4ade80";

  document.getElementById("step-4-badge").textContent = "Sample: completed field";
...
window.verifyVisualReceiptFlow = function() {
  ...
  alert("Receipt sample inspection:\n\n" +
        "1. Voter EIP-191 Envelope: SAMPLE SHAPE (binds chain ID 31337 and proxy address fields)\n" +
        "2. Relayer EIP-712 Attestation: SAMPLE SHAPE (trusted issuer and bytes32 policy fields present)\n" +
        "3. Privacy Blinding: SAMPLE ONLY (domain-separated keyed hash; production unlinkability is not proven here)\n" +
        "4. Request Ledger State: SAMPLE ONLY (completed status field present; no live database lock is proven here)\n\n" +
        "Sample receipt fields are structurally present.");
};
...
window.initFiduciaryCopilot = function() {
  ...
  async function handleSendQuery(queryText) {
    ...
    try {
      ...
      botMsg.textContent = "Offline local fallback: no live RAG endpoint is connected. Consult local Hardhat tests and governance dossiers for evidence.";
    }
    messagesEl.scrollTop = messagesEl.scrollHeight;
  }

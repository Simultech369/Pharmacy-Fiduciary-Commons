...
| Patient fund receives a configured share of gross claims, default 10% | Contract-enforced | `PBMRebateTreasury.claim` routes `patientClaimBP` within the 5%-30% range | Add dashboard visibility for patient-fund inflows and rate changes |
...
| Claim | Current Coverage | Evidence | Gap / Next Step |
|-------|------------------|----------|-----------------|
| Tests run locally | Tested | `npm.cmd test` currently passes 280 tests | Keep CI green |
...
| Production frontend build exists | Verified in working tree | `npm.cmd run build:dashboard` & `check:frontend` | 0 inline styles, bundled assets in dist/dashboard/ |
| Database/RLS exists | Gate DB1 Verified | `test/server.test.js` (20/20 passing) | `server/createApp.js` EIP-191/712 security and multi-tenant RLS policy behavior are verified through a JS simulation harness; production schema-backed PostgREST RLS execution remains a separate deployment verification gate |

...
| **2. Database & Auth Boundaries** | Local-only prototype with synthetic/mock data. Express proxy server (`server/createApp.js`) implemented with EIP-191 voter sync signature verification, nonce tracking replay protection, and Supabase database handlers. | None. | **Fix Now** (Implemented)<br>**Backlog** (Hosted DB) | Conduct trust boundary review when a hosted database or auth provider (e.g. Supabase, Firebase) is proposed in production. | **Partially Closed** |
| **3. Offline Workflow Safety** | [continuity-engine.mjs](file:///c:/Users/Josh/Desktop/PBMRebateTreasuryFinal/tools/resilience/continuity-engine.mjs) handles offline vouchers with all-key MAC coverage. Deterministic reconciliation via [reconcile-vouchers.mjs](file:///c:/Users/Josh/Desktop/PBMRebateTreasuryFinal/tools/resilience/reconcile-vouchers.mjs) and tests pass. Detailed tabletop playbooks for paper, SMS, delayed-sync, and duplicate/forged receipts compiled in [dossier_non_digital_scenarios.md](file:///c:/Users/Josh/Desktop/PBMRebateTreasuryFinal/review-context/dossier_non_digital_scenarios.md). | Console output "Verified Status: TRUE" overclaimed local MAC strength (Kimi Finding 3). Offline MACs are truncated to 16 hex chars (64 bits), representing short local tamper checks, not strong receipt finality or fraud-proofing (Finding 3). Finding 5 flags that full reconciliation remains a launch blocker, not just tabletop polish. | **Fix Now** (Implemented)<br>**Document** | Keep `LOCAL INTEGRITY ONLY (NOT A ZK PROOF)` wording and enforce 64-bit MAC strength boundaries. | **Closed / Monitor** |
...
| **13. Contract Security & Pause Gates** | [PBMRebateTreasury.sol](file:///c:/Users/Josh/Desktop/PBMRebateTreasuryFinal/contracts/PBMRebateTreasury.sol) contains pause gates. resolveClaim refactored to Checks-Effects-Interactions clean layout and hasClaimed reset fixed for dismissed exclusion disputes. [SCANNER_TRIAGE.md](file:///c:/Users/Josh/Desktop/PBMRebateTreasuryFinal/SCANNER_TRIAGE.md) is populated. | `updateSanction` bypassed guardian pause (Kimi Finding 4). | **Fix Now** / **Test Now** (Implemented)<br>**Backlog** (Scanner refresh) | Added `whenNotPaused` modifier to `updateSanction` and verified passing tests. Rerun Slither/Aderyn/targeted scanner checks before release claims. | **Closed / Monitor** |

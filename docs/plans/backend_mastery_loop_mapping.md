...
| **Stage 2: Data Layer** | Relational schema, SQL, normalization, indexes, transactions, ORM/raw SQL tradeoffs | Supabase/Postgres ledger design, RLS tenant boundaries, idempotency lease RPCs, saga queue SQL migration | `test/server.test.js`, `supabase/schema.sql`, `supabase/migrations/20260813000000_voucher_settlement_saga.sql` | Live Supabase migration execution, isolation-level tests, and production query plans remain future work. |
...
| **Stage 4: Scale & Reliability** | Stateless scaling, load balancing, database scaling, message brokers, rate limiting, observability, resilience patterns | Scoped observability envelope, request throttling, outage tests, local voucher saga endpoint and prototype saga telemetry | `GET /api/health/observability`, `test/DisasterRecoveryOutage.test.js`, `test/VoucherSagaQueue.test.js`, `test/Phase6Operationalization.test.js` | Live Supabase metric readers, Prometheus/OpenTelemetry export, production multi-instance stress tests, and real broker-backed queues remain open. |
| **Stage 5: Distributed-Systems Mindset** | CAP tradeoffs, sagas, concurrency/locking, backend security, profiling, multi-tenancy | Solvency invariants, zero-toll dispute retraction, local multi-instance saga lease test, RLS isolation rules, scanner triage | `contracts/PBMRebateTreasury.sol`, `test/PBMRebateTreasury.dispute-timeout.test.js`, `test/VoucherSagaQueue.test.js`, `docs/ops/SCANNER_TRIAGE.md` | Production concurrency protection still needs live database isolation tests and deployment stress testing. |
...
2. **Verify Stage 2 + Stage 5 together**: run the durable voucher saga queue tests covering transaction shape, idempotency, RLS text, and concurrency lease behavior.
3. **Expand Stage 4 after local telemetry exists**: add live database metric readers, Prometheus/OpenTelemetry export, and multi-instance lease observability.
...
| **P2** | Live Supabase voucher saga migration test | Moves the local schema/RPC proof beyond mocked Supabase. |
| **P3** | Production-style multi-instance lease stress test | Targets the highest-value remaining distributed-systems race. |
| **P4** | Executed RLS tenant isolation test for saga rows | Ensures pharmacy-level data boundaries survive a real database policy check. |
| **P5** | Live observability metric reader test | Moves prototype in-memory queue metrics beyond local test snapshots without inventing fake live financial readings. |
